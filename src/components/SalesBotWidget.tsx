  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          {/* Header */}
          <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0" style={{ background: "#5645d4" }}>
            <div
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3" style={{ background: "var(--surface)" }}>
            {messages.map(msg => (
            ...
            {loading && !streaming && (
              <div className="flex justify-start">
                <div className="px-4 py-3 rounded-xl flex gap-2 items-center" style={{ background: "var(--canvas)", border: "1px solid var(--hairline)" }}>
                  <div className="flex gap-1.5">
                    {[0, 1, 2].map(i => (
                      <div key={i} className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ background: "#5645d4", animationDelay: `${i * 0.15}s` }} />
                    ))}
                  </div>
                  {toolStatus && <span className="text-xs" style={{ color: "var(--steel)" }}>{toolStatus}</span>}
                </div>
              </div>
            )}
            <div ref={bottomRef} />
          ...
          {/* Input */}
          <div className="flex gap-2 px-3 py-3 flex-shrink-0" style={{ background: "var(--canvas)", borderTop: "1px solid var(--hairline)" }}>
            <input
              ref={inputRef}
              value={input}
            ...
            <button
              ...
              style={{ background: input.trim() && !loading ? "#5645d4" : "var(--hairline)" }}
            >
      ...
      <button
        ...
        style={{ background: "#5645d4", boxShadow: "rgba(86,69,212,0.4) 0px 6px 20px 0px" }}
      >
        {open ? (
