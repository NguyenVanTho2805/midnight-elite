    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.4)" }}>
      <div className="bg-white rounded-xl w-full max-w-3xl max-h-[85vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: "var(--hairline)" }}>
          <div>
            <h2 className="text-base font-bold" style={{ color: "var(--ink)" }}>Quét trùng lặp</h2>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-4">
          {loading ? (
            ...
            groups.map((group, gi) => (
              <div key={gi} className="rounded-lg border overflow-hidden" style={{ borderColor: "var(--hairline)" }}>
                <div className="px-3 py-2 text-xs font-semibold" style={{ background: "var(--surface)", color: "var(--steel)" }}>
                  ...
                  {group.items.map(item => (
                    <div key={item.id} className="flex items-start gap-3 px-3 py-2.5">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm truncate" style={{ color: "var(--ink)" }}><MathText text={item.text} /></p>
        ...
        <div className="flex justify-end gap-2 px-5 py-4 border-t" style={{ borderColor: "var(--hairline)" }}>
          <button onClick={onClose} className="px-4 py-2 text-sm border rounded-lg text-gray-600 hover:bg-gray-50" style={{ borderColor: "var(--hairline)" }}>Đóng</button>
        </div>
      </div>
    </div>
  );
