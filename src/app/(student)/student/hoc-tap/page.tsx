...
import { CaretDown as ChevronDown, Play, Eye, Lock, ArrowRight, BookOpen } from "@phosphor-icons/react";
...
function TypeBadge({ type }: { type: string }) {
  const map: Record<string, { label: string; color: string; bg: string }> = {
    record:   { label: "Video",    color: "#5645d4", bg: "var(--tint-lavender)" },
    ...
    if (enrollmentsLoading || enrolledIds.size > 0) {
      return (
        <div className="space-y-4">
          <div className="h-7 w-48 rounded-xl animate-pulse" style={{ background: "var(--hairline)" }} />
          <div className="h-24 rounded-xl animate-pulse" style={{ background: "var(--hairline)" }} />
        </div>
      );
    }
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4">
        <BookOpen size={40} style={{ color: "var(--stone)" }} />
        <p className="text-sm" style={{ color: "var(--stone)" }}>Bạn chưa có khóa học nào.</p>
        <Link href="/khoa-hoc" className="text-sm font-semibold" style={{ color: "#5645d4" }}>← Khóa học</Link>
      </div>
    );
  }
      ...
      <div className="space-y-4">
        ...
        {[1,2,3].map(i => (
          <div key={i} className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--hairline)" }}>
            <div className="h-10 animate-pulse" style={{ background: "var(--surface)" }} />
            {[1,2,3].map(j => (
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm font-medium truncate" style={{ color: "var(--stone)" }}>{course.name}</span>
      </div>

      {/* Course header card */}
      <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--hairline)" }}>
        <div className="h-2" style={{ background: course.strip || "#5645d4" }} />
        <div className="p-5 flex items-start gap-4" style={{ background: "var(--canvas)" }}>
          <div className="flex-1 min-w-0">
            <p className="text-xs mb-1" style={{ color: "var(--stone)" }}>{course.category}</p>
            <h1 className="text-xl font-extrabold leading-snug mb-1" style={{ color: "var(--ink)" }}>{course.name}</h1>
            <p className="text-sm" style={{ color: "var(--steel)" }}>Gia sư: {course.instructor}</p>
            <div className="flex items-center gap-4 mt-3 text-xs" style={{ color: "var(--stone)" }}>
          ...
          <div className="flex-shrink-0 text-center">
            <p className="text-2xl font-extrabold" style={{ color: "#5645d4" }}>{pct}%</p>
            <p className="text-xs" style={{ color: "var(--stone)" }}>hoàn thành</p>
            <p className="text-xs mt-1 font-semibold" style={{ color: "var(--steel)" }}>{completedCount}/{allLessons.length} bài</p>
          </div>
        </div>

        {/* Progress bar */}
        <div className="h-1.5 mx-5 mb-5 rounded-full overflow-hidden" style={{ background: "var(--hairline)" }}>
        ...
        <div className="px-5 pb-5">
          {isEnrolled === false ? (
            <div className="space-y-2">
              <p className="text-xs text-center" style={{ color: "var(--stone)" }}>Bạn chưa đăng ký khóa học này</p>
      ...
      {/* Sections */}
      <div className="space-y-2">
        <p className="text-sm font-extrabold" style={{ color: "var(--ink)" }}>Nội dung khóa học</p>
        {course.sections.map(section => {
          const lessons = section.chapters.flatMap(c => c.lessons);
          const done    = lessons.filter(l => completedIds.has(l.id)).length;
          const isOpen  = openSections.has(section.id);
          return (
            <div key={section.id} className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--hairline)" }}>
              <button onClick={() => toggleSection(section.id)}
                className="w-full flex items-center gap-3 px-4 py-3 text-left"
                style={{ background: "var(--surface)" }}>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-extrabold uppercase tracking-wide" style={{ color: "var(--ink)" }}>
                    {section.title}
                  </p>
                  <p className="text-xs mt-0.5" style={{ color: "var(--stone)" }}>{done}/{lessons.length} bài hoàn thành</p>
                </div>
                  ...
                  {section.chapters.map((chapter, ci) => (
                    <div key={chapter.id}>
                      {section.chapters.length > 1 && (
                        <div className="px-4 py-2" style={{ background: "var(--surface-soft)", borderBottom: "1px solid #f0ede9" }}>
                      ...
                      {chapter.lessons.map(lesson => {
                        ...
                        return (
                          <button key={lesson.id}
                            ...
                            className="w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-[var(--surface-soft)]"
                            style={{
                              borderBottom: "1px solid #f0ede9",
                              borderLeft: isNext ? "3px solid #5645d4" : "3px solid transparent",
                              background: isNext ? "var(--tint-lavender)" : "transparent",
                            ...
                            <div className="flex-shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center"
                              style={{
                                borderColor: isDone ? "#00A63D" : isNext ? "#5645d4" : "#d1d5db",
                                background: isDone ? "#00A63D" : isNext ? "var(--tint-lavender)" : "transparent",
                              }}>
                              {isDone && (
                              ...
                              {!isDone && lesson.isLocked && <Lock size={8} style={{ color: "var(--stone)" }} />}
                            </div>

                            {/* Lesson info */}
                            <div className="flex-1 min-w-0">
                              <p className="text-sm leading-snug truncate"
                                style={{ color: isNext ? "#5645d4" : "var(--charcoal)", fontWeight: isDone || isNext ? 600 : 400 }}>
                                {lesson.title}
                              </p>
                              <div className="flex items-center gap-2 mt-1">
                                <TypeBadge type={lesson.type} />
                                {lesson.duration && (
                                  <span className="text-xs flex items-center gap-1" style={{ color: "var(--hairline-strong)" }}>
