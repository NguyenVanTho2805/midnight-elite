...
function Avatar({ name, size = 44 }: { name: string; size?: number }) {
  const colors = ["#4534b3", "#7C3AED", "#16a34a", "#EF4444", "#FE9900", "#0891B2"];
...
function TabThiThu() {
  ...
  return (
    <>
      ...
      {loading && <div className="space-y-2">{[1,2,3,4,5].map(i => <div key={i} className="h-14 rounded-xl animate-pulse" style={{ background:"var(--surface)" }} />)}</div>}
      ...
      {!loading && !fetchError && leaderboard.length === 0 && (
        <div className="rounded-xl p-12 mb-8 text-center" style={{ background: "var(--canvas)", border: "1px solid var(--hairline)" }}>
          <p className="text-base font-semibold mb-1" style={{ color: "var(--ink)" }}>Chưa có kết quả cho {activeFilter}</p>
          <button onClick={() => setActiveFilter("Tất cả")} className="mt-3 text-sm font-semibold" style={{ color: "#5645d4" }}>
            Xem tất cả →
          </button>
        </div>
      )}
      ...
      {top3.length >= 3 && (
        <div className="grid grid-cols-3 gap-4 mb-10 items-end">
          ...
          <div className="text-center">
            <div className="rounded-xl p-4 mx-auto" style={{ background: "var(--canvas)", border: "1px solid var(--hairline)" }}>
              <div className="flex justify-center mb-2"><span className="text-2xl font-bold" style={{ color: "var(--stone)" }}>2</span></div>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-lg mx-auto mb-3"
                style={{ background: "linear-gradient(145deg,var(--stone),var(--steel))" }}>{top3[1].name[0]}</div>
              <p className="text-xs font-bold mb-0.5 truncate" style={{ color: "var(--ink)" }}>{top3[1].name}</p>
              <p className="text-xs mb-2 truncate" style={{ color: "var(--stone)" }}>{top3[1].school.split(",")[0]}</p>
              <p className="text-lg font-bold" style={{ color: "var(--steel)" }}>{top3[1].score}</p>
            </div>
            <div className="h-12 flex items-end justify-center"><span className="text-xs font-semibold" style={{ color: "var(--stone)" }}>Hạng 2</span></div>
          </div>
          {/* #1 */}
          ...
          <div className="text-center">
            <div className="rounded-xl p-4 mx-auto" style={{ background: "var(--canvas)", border: "1px solid var(--hairline)" }}>
              ...
              <p className="text-xs font-bold mb-0.5 truncate" style={{ color: "var(--ink)" }}>{top3[2].name}</p>
              <p className="text-xs mb-2 truncate" style={{ color: "var(--stone)" }}>{top3[2].school.split(",")[0]}</p>
              <p className="text-lg font-bold" style={{ color: "#CD7F32" }}>{top3[2].score}</p>
            </div>
            <div className="h-12 flex items-end justify-center"><span className="text-xs font-semibold" style={{ color: "var(--stone)" }}>Hạng 3</span></div>
          </div>
        </div>
      )}

      {leaderboard.length > 0 && (
        <div className="rounded-xl overflow-hidden mb-8" style={{ background: "var(--canvas)", border: "1px solid var(--hairline)" }}>
          ...
          {leaderboard.map((s, idx) => {
            ...
            return (
              <div key={`${s.name}-${idx}`} className="grid grid-cols-12 items-center px-6 py-4"
                style={{
                  borderBottom: idx < leaderboard.length - 1 ? "1px solid var(--hairline)" : "none",
                  background: isMe ? "var(--tint-lavender)" : idx < 3 ? "#fafafa" : "transparent",
                  borderLeft: isMe ? "3px solid #5645d4" : "3px solid transparent",
                }}>
                <div className="col-span-1 flex items-center">
                  {idx === 0 ? <Trophy size={20} style={{ color: "#b45309" }} /> : (
                    <span className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold"
                      style={{ background: "var(--surface)", border: "1px solid var(--hairline)", color: idx < 3 ? "#5645d4" : "var(--steel)" }}>
                      {idx + 1}
                    </span>
                  )}
                </div>
                <div className="col-span-5 flex items-center gap-2">
                  <div className="font-semibold text-sm" style={{ color: isMe ? "#5645d4" : "var(--ink)" }}>{s.name}</div>
                  {isMe && (
                    <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full text-white" style={{ background: "#5645d4" }}>Bạn</span>
                  )}
                </div>
                <div className="col-span-3 hidden sm:block text-xs" style={{ color: "var(--stone)" }}>{s.school.split(",")[0]}</div>
                <div className="col-span-3 text-right">
                  <span className="text-base font-bold" style={{ color: "#5645d4" }}>{s.score}</span>
  ...
  return (
    <>
      <div className="flex items-center justify-between flex-wrap gap-3 mb-8">
        <p className="text-sm" style={{ color: "var(--steel)" }}>{students.length} học sinh Midnight Elite</p>
      ...
      {loading && <div className="space-y-2">{[1,2,3,4,5].map(i => <div key={i} className="h-16 rounded-xl animate-pulse" style={{ background:"var(--surface)" }} />)}</div>}
      ...
      {!loading && !fetchError && students.length === 0 && (
        <div className="text-center py-12 rounded-xl" style={{ background: "var(--canvas)", border: "1px solid var(--hairline)" }}>
          <p className="text-sm" style={{ color: "var(--stone)" }}>Chưa có dữ liệu vinh danh.</p>
        </div>
      )}
      ...
      {top3.length >= 3 && (
        <div className="grid grid-cols-3 gap-4 mb-10 items-end">
          <div className="text-center">
            <div className="rounded-xl p-5 mx-auto" style={{ background: "var(--canvas)", border: "1px solid var(--hairline)" }}>
              <div className="text-2xl font-bold mb-3" style={{ color: "var(--stone)" }}>2</div>
              <div className="flex justify-center mb-3"><Avatar name={top3[1].name} size={48} /></div>
              <p className="text-xs font-bold mb-0.5 truncate" style={{ color: "var(--ink)" }}>{top3[1].name}</p>
              <p className="text-xs mb-2 truncate" style={{ color: "var(--stone)" }}>{top3[1].school.split(",")[0]}</p>
              <div className="text-xl font-bold" style={{ color: "var(--steel)" }}>
                {sortKey === "gpa" ? top3[1].gpa.toFixed(1) : sortKey === "lastExamScore" ? top3[1].lastExamScore : `${top3[1].completion}%`}
              </div>
            </div>
            <div className="h-8 flex items-end justify-center"><span className="text-xs font-semibold" style={{ color: "var(--stone)" }}>Hạng 2</span></div>
          </div>
          ...
          <div className="text-center">
            ...
            <div className="h-8 flex items-end justify-center"><span className="text-xs font-semibold" style={{ color: "var(--stone)" }}>Hạng 3</span></div>
          </div>
        </div>
      )}

      {ranked.length > 0 && (
        <div className="rounded-xl overflow-hidden mb-10" style={{ background: "var(--canvas)", border: "1px solid var(--hairline)" }}>
          <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid var(--hairline)", background: "var(--surface)" }}>
            <h2 className="font-semibold text-sm" style={{ color: "var(--ink)" }}>Bảng xếp hạng đầy đủ</h2>
          ...
          {ranked.map((s, idx) => {
            ...
            return (
              <div key={s.id} className="flex items-center gap-4 px-6 py-4"
                style={{
                  borderBottom: idx < ranked.length - 1 ? "1px solid var(--hairline)" : "none",
                  background: isMe ? "var(--tint-lavender)" : idx < 3 ? "#fafafa" : "transparent",
                  borderLeft: isMe ? "3px solid #5645d4" : "3px solid transparent",
                }}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="font-semibold text-sm" style={{ color: isMe ? "#5645d4" : "var(--ink)" }}>{s.name}</span>
                    {isMe && (
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full text-white" style={{ background: "#5645d4" }}>Bạn</span>
                    )}
                    {badges.map(b => (
                  ...
                  <p className="text-xs mt-0.5 truncate" style={{ color: "var(--stone)" }}>{s.school}</p>
                </div>
                <div className="hidden sm:flex items-center gap-5 flex-shrink-0">
                  <div className="w-24"><GpaBar value={s.gpa} /><p className="text-xs text-center mt-0.5" style={{ color: "var(--stone)" }}>GPA</p></div>
                  <div className="text-center hidden md:block">
                    <div className="text-sm font-bold" style={{ color: "#5645d4" }}>{s.lastExamScore > 0 ? s.lastExamScore : "—"}</div>
                    <p className="text-xs" style={{ color: "var(--stone)" }}>Điểm thi</p>
                  </div>
                  <div className="text-center hidden lg:block">
                    <div className="text-sm font-bold" style={{ color: "#7C3AED" }}>{s.completion}%</div>
                    <p className="text-xs" style={{ color: "var(--stone)" }}>Tiến độ</p>
                  </div>
                  <div className="text-center hidden lg:block">
                    <div className="text-sm font-bold" style={{ color: "#16a34a" }}>{s.assignmentAvg != null ? s.assignmentAvg : "—"}</div>
                    <p className="text-xs" style={{ color: "var(--stone)" }}>Điểm bài tập</p>
                  </div>
                </div>
              </div>
            );
          })}
          <div className="px-6 py-4 text-center" style={{ borderTop: "1px solid var(--hairline)", background: "var(--surface)" }}>
            <p className="text-xs" style={{ color: "var(--stone)" }}>GPA = 40% tiến độ + 60% điểm thi · Cập nhật tự động từ DB</p>
          </div>
        </div>
      )}
      ...
      {/* Badges */}
      <div className="mb-10">
        <h2 className="text-xl font-bold mb-6 text-center" style={{ color: "var(--ink)", letterSpacing: "-0.3px" }}>
          Badge <span style={{ color: "#b45309" }}>tháng này</span>
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {badgesWithStudents.map(b => (
            <div key={b.id} className="rounded-xl p-5" style={{ background: "var(--canvas)", border: "1px solid var(--hairline)" }}>
              <div className="flex items-center gap-3 mb-4">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl flex-shrink-0"
                  style={{ background: b.bg, border: `1px solid ${b.borderColor}` }}>{b.icon}</div>
                <div>
                  <p className="font-bold text-sm" style={{ color: "var(--ink)" }}>{b.title}</p>
                  <p className="text-xs" style={{ color: "var(--stone)" }}>{b.desc}</p>
                  <p className="text-xs font-semibold mt-0.5" style={{ color: b.color }}>{b.reward}</p>
                </div>
              </div>
              {b.students.length === 0 ? (
                <p className="text-xs text-center py-2" style={{ color: "var(--stone)" }}>Chưa có học sinh đủ điều kiện</p>
              ) : (
                <div className="space-y-2">
                  {b.students.map((s, i) => (
                    <div key={s.id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg"
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold truncate" style={{ color: "var(--ink)" }}>{s.name}</p>
                        <p className="text-xs truncate" style={{ color: "var(--stone)" }}>{s.school.split(",")[0]}</p>
                      </div>
