...
const categoryTheme: Record<string, string> = {
  "ĐGNL HSA":        "linear-gradient(135deg,#3a2a99,#5645d4,#38BDF8)",
  ...
  "BCA":             "linear-gradient(135deg,var(--ink),#374151,var(--steel))",
};
      ...
      <div className="flex items-center justify-center px-4 py-20">
        <div className="text-center max-w-sm">
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-5"
              ...
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--stone)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/>
                <path d="M1 1h4l2.68 13.39a2 2 0 001.98 1.61H19a2 2 0 001.97-1.67L23 6H6"/>
              </svg>
            </div>
            <h1 className="text-xl font-bold mb-2" style={{ color: "var(--ink)" }}>Đăng nhập để xem khóa học đã lưu</h1>
            <p className="text-sm mb-6" style={{ color: "var(--steel)" }}>Khóa học được lưu theo tài khoản của bạn</p>
  ...
  return (
    <div>
      ...
      <div className="max-w-5xl mx-auto w-full px-4 py-10">
        ...
        {/* Header */}
        <div className="mb-7">
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: "var(--ink)", letterSpacing: "-0.5px" }}>
            Khóa học đã lưu
            {!loading && items.length > 0 && (
              <span className="ml-2 text-sm font-semibold px-2 py-0.5 rounded-full align-middle"
                style={{ background: "#5645d4", color: "#fff" }}>
                {items.length}
              </span>
            )}
          </h1>
          <p className="text-sm mt-1" style={{ color: "var(--steel)" }}>Các khóa học bạn đã lưu lại</p>
        </div>
          ...
          <div className="space-y-3">
            {[1, 2].map(i => (
              <div key={i} className="rounded-xl p-4 animate-pulse flex gap-4"
                style={{ background: "var(--canvas)", border: "1px solid var(--hairline)" }}>
                <div className="w-20 h-20 rounded-lg flex-shrink-0" style={{ background: "var(--hairline)" }} />
                <div className="flex-1 space-y-2">
                  <div className="h-4 w-3/4 rounded" style={{ background: "var(--hairline)" }} />
                  <div className="h-3 w-1/2 rounded" style={{ background: "var(--tint-gray)" }} />
                  <div className="h-4 w-1/4 rounded" style={{ background: "var(--hairline)" }} />
                </div>
              </div>
            ))}
          </div>
        ...
        {!loading && items.length === 0 && (
          <div className="rounded-xl py-20 text-center"
            ...
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-5"
              ...
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--hairline-strong)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/>
                <path d="M1 1h4l2.68 13.39a2 2 0 001.98 1.61H19a2 2 0 001.97-1.67L23 6H6"/>
              </svg>
            </div>
            <p className="font-semibold mb-1" style={{ color: "var(--ink)" }}>Chưa có khóa học nào được lưu</p>
            <p className="text-sm mb-6" style={{ color: "var(--stone)" }}>Khám phá các khóa học và lưu lại để học sau</p>
            <Link href="/khoa-hoc"
          ...
          <div className="flex flex-col lg:flex-row gap-5 items-start">
                  ...
                  <div key={courseId}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        ...
                        {course.originalPrice && course.originalPrice > course.price && (
                          <span className="text-xs line-through" style={{ color: "var(--hairline-strong)" }}>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <Link href={`/khoa-hoc/${courseId}`}
                        ...
                        style={{ color: "#5645d4", border: "1px solid var(--brand-purple-300)" }}>
                        Chi tiết
                      </Link>
                      <button
            ...
            <div className="lg:w-72 w-full flex-shrink-0">
              <div className="rounded-xl p-5 sticky top-20"
                style={{ background: "var(--canvas)", border: "1px solid var(--hairline)" }}>
                <p className="text-sm font-bold mb-4" style={{ color: "var(--ink)" }}>Tóm tắt đơn hàng</p>

                <div className="space-y-2.5 mb-4">
                  {items.map(({ courseId, course }) => (
                    <div key={courseId} className="flex justify-between text-xs">
                      <span className="truncate max-w-[150px]" style={{ color: "var(--steel)" }}>{course.name}</span>
                ...
                <div className="pt-3 mb-5" style={{ borderTop: "1px solid var(--hairline)" }}>
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-bold" style={{ color: "var(--ink)" }}>Tổng cộng</span>
                    <span className="text-lg font-black" style={{ color: "#5645d4" }}>
                      {formatPrice(total)}
                    </span>
                  </div>
                  <p className="text-xs mt-1" style={{ color: "var(--stone)" }}>Đã bao gồm VAT (nếu có)</p>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-4 pb-4 sm:pb-0"
          ...
          <div className="w-full max-w-md rounded-2xl p-6"
            ...
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-bold" style={{ color: "var(--ink)" }}>Hoàn tất đăng ký</h2>
              <button onClick={() => setCheckoutOpen(false)}
                className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-[var(--surface)]"
                style={{ color: "var(--steel)" }}>✕</button>
            </div>

            <div className="rounded-xl p-4 mb-5" style={{ background: "var(--surface)", border: "1px solid var(--hairline)" }}>
              <p className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: "var(--stone)" }}>Tổng thanh toán</p>
              <p className="text-2xl font-black" style={{ color: "#5645d4" }}>{formatPrice(total)}</p>
              <p className="text-xs mt-1" style={{ color: "var(--steel)" }}>{items.length} khóa học</p>
            </div>
            ...
            <div className="space-y-3">
              <a href="https://zalo.me/0384409051" target="_blank" rel="noopener noreferrer"
                ...
                style={{ background: "#5645d4", color: "#ffffff" }}>
              ...
              <a href="tel:0384409051"
                className="flex items-center gap-3 w-full px-4 py-3 rounded-xl transition-colors hover:bg-[var(--surface)]"
                style={{ border: "1px solid var(--hairline)" }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--ink)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.8 19.79 19.79 0 01.02 1.18C.02.57.47.03 1.08.01h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L5.09 7.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>
                <div>
                  <p className="text-sm font-semibold" style={{ color: "var(--ink)" }}>Gọi hotline</p>
                  <p className="text-xs" style={{ color: "var(--steel)" }}>0384 409 051 · 9:00 – 21:30</p>
                </div>
              </a>
            </div>

            <p className="text-xs text-center mt-4" style={{ color: "var(--stone)" }}>
