...
function ItemDrawer({ open, initial, isEdit, categories, onCategoriesChange, onClose, onSave, saving }: {
  open: boolean;
  initial: ItemForm;
  isEdit: boolean;
      ...
      <div className="fixed top-0 right-0 bottom-0 z-50 bg-white overflow-y-auto shadow-2xl"
        ...
        <div className="sticky top-0 bg-white z-10 flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: "#e5e3df" }}>
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-bold" style={{ color: "#1a1a1a" }}>{isEdit ? "Sửa câu hỏi" : "Thêm câu hỏi vào ngân hàng"}</h2>
          </div>
...
function PageInner() {
  ...
  // "Tự chọn câu hỏi" — chế độ chọn nhiều để gộp vào 1 đề đang có. Map (không
  // phải Set) vì cần giữ nguyên QuestionBankItemFull đã chọn để map thẳng
  // sang ExamQuestionInput lúc thêm, không phải gọi lại API.
  ...
  return (
    <div className={`p-6 max-w-7xl mx-auto ${selectMode && selected.size > 0 ? "pb-24" : ""}`}>
      <p className="text-sm text-gray-500 mb-1">
        ...
        {" "}/ <span className="font-medium text-gray-800">Danh sách câu hỏi</span>
      </p>
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-xl font-extrabold" style={{ color: "#1a1a1a" }}>
            {activeCategoryName ? `Câu hỏi trong "${activeCategoryName}"` : "Danh sách câu hỏi"}
          </h1>
      ...
      <div className="rounded-xl border overflow-hidden" style={{ borderColor: "#e5e3df" }}>
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={selectMode ? 10 : 9} className="px-4 py-8 text-center text-gray-400">Đang tải...</td></tr>
            ) : items.length === 0 ? (
              <tr><td colSpan={selectMode ? 10 : 9} className="px-4 py-8 text-center text-gray-400">Chưa có câu hỏi nào</td></tr>
            ) : items.map(item => (
      ...
      <ItemDrawer
        open={drawerOpen}
        initial={editItem ? toForm(editItem) : { ...INIT_FORM, categoryId: categoryFilter }}
        isEdit={!!editItem}
