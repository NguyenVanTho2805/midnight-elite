...
import { ChartBar, ClipboardText as ClipboardList } from "@phosphor-icons/react";
    ...
    <PermissionGuard required={PERMISSIONS.VIEW_REVENUE}>
      ...
      <div className="space-y-6 overflow-y-auto" style={{ height: "calc(100vh - 104px)" }}>
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-extrabold" style={{ color: "var(--ink)" }}>Doanh thu</h1>
            <p className="text-sm mt-1" style={{ color: "var(--steel)" }}>
              Tính theo kích hoạt khoá học · Mỗi enrollment = 1 giao dịch
            </p>
          </div>
          <button onClick={loadAnalytics} className="notion-btn-secondary text-sm">
            ↺ Tải lại
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <div key={c.label} className="rounded-lg p-5"
              style={{ background: "var(--canvas)", border: "1px solid var(--hairline)" }}>
              <div className="mb-2">
                <div className="text-xs" style={{ color: "var(--steel)" }}>{c.label}</div>
              </div>
              <div className="text-2xl font-extrabold mb-1" style={{ color: c.color }}>{c.value}</div>
              <div className="text-xs" style={{ color: "var(--stone)" }}>{c.sub}</div>
            </div>
          ))}
        </div>
          ...
          {loading ? (
            <div className="space-y-3">
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="h-8 rounded-md animate-pulse" style={{ background: "var(--hairline)" }} />
              ))}
            </div>
          ) : (analytics?.byCourse ?? []).length === 0 ? (
            <p className="text-sm text-center py-6" style={{ color: "var(--stone)" }}>Chưa có dữ liệu doanh thu</p>
          ) : (
            (analytics?.byCourse ?? []).map(p => (
              <div key={p.name} className="flex items-center gap-4 mb-3">
                <div className="w-44 flex-shrink-0 text-sm truncate" style={{ color: "var(--ink)" }}>{p.name}</div>
                <div className="flex-1 h-2 rounded-full" style={{ background: "var(--surface)" }}>
                  <div className="h-2 rounded-full"
                    style={{ width: `${(p.revenue / chartMax * 100).toFixed(1)}%`, background: "linear-gradient(90deg,#5645d4,#7b3ff2)" }} />
                </div>
                <div className="text-right flex-shrink-0 w-32">
                  <div className="text-sm font-bold" style={{ color: "#5645d4" }}>
                    {(p.revenue / 1_000_000).toFixed(1)}Mđ
                  </div>
                  <div className="text-xs" style={{ color: "var(--stone)" }}>{p.enrollments} học viên</div>
                </div>
              </div>
            ))
          )}
            ...
            <div className="space-y-3">
              {(analytics?.revenueByMonth ?? []).map(m => (
                <div key={m.month} className="flex items-center gap-4">
                  <div className="w-16 flex-shrink-0 text-xs font-mono" style={{ color: "var(--steel)" }}>
                  ...
                  <div className="text-right flex-shrink-0 w-28">
                    ...
                    <div className="text-xs" style={{ color: "var(--stone)" }}>{m.enrollments} kích hoạt</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="rounded-lg p-6" style={{ background: "var(--canvas)", border: "1px solid var(--hairline)" }}>
          <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <select value={catFilter} onChange={e => setCatFilter(e.target.value)}
                className="notion-input text-xs" style={{ height: "auto", padding: "6px 12px" }}>
                <option value="">Tất cả khoá học</option>
                {(analytics?.byCourse ?? []).map(c => (
                  <option key={c.name} value={c.name}>{c.name}</option>
                ))}
              </select>
              <button onClick={exportCSV} className="notion-btn-secondary text-xs">
                ↓ Export CSV
              </button>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <p className="text-center py-10 text-sm" style={{ color: "var(--stone)" }}>Chưa có giao dịch nào</p>
          ) : (
            <div className="space-y-2">
              {displayEnrollments.map(e => (
                <div key={e.id} className="grid grid-cols-12 items-center px-4 py-3 rounded-md"
                  style={{ background: "var(--surface)" }}>
                  <div className="col-span-3">
                    <div className="text-sm font-medium" style={{ color: "var(--ink)" }}>{e.userName}</div>
                    <div className="text-xs" style={{ color: "var(--stone)" }}>{e.userPhone}</div>
                  </div>
                  <div className="col-span-4 text-xs truncate" style={{ color: "var(--slate)" }}>{e.courseName}</div>
                  <div className="col-span-2 text-right font-bold text-sm" style={{ color: "#5645d4" }}>
                  ...
                  <div className="col-span-1 text-right text-xs" style={{ color: "var(--stone)" }}>
          ...
          <div className="mt-4 pt-3 flex items-center justify-between" style={{ borderTop: "1px solid var(--hairline)" }}>
            <p className="text-xs" style={{ color: "var(--stone)" }}>
              Hiển thị {displayEnrollments.length} giao dịch
            </p>
            <p className="text-xs font-semibold" style={{ color: "#5645d4" }}>
