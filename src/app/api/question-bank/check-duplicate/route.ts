...
// 3 cấp đều thu hẹp theo categoryId trước khi so (đúng tối ưu bắt buộc
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { text, categoryId } = await req.json() as { text?: string; categoryId?: string };
    if (!text?.trim() || !categoryId?.trim()) {
      return NextResponse.json({ error: "Thiếu nội dung/đầu mục" }, { status: 400 });
    }
    ...
    const match = await prisma.questionBankItem.findFirst({
      where: { ...statusScope, categoryId, contentHash: computeContentHash(text) },
      include: { options: { orderBy: { order: "asc" } }, owner: { select: { name: true } } },
    });

    const similar = match
      ? []
      : await findSimilarBankItems({ text, categoryId, userId: auth.userId, isReviewer: isReviewer(auth.adminRole) });

    const semantic = match || similar.length > 0
      ? []
      : await findSemanticBankItems({ text, categoryId, userId: auth.userId, isReviewer: isReviewer(auth.adminRole) });
